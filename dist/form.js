...
function uniqueDefaultTag(existingTags) {
    ...
    if (!taken.has("MTProto"))
        return "MTProto";
    let n = 2;
    while (taken.has(`MTProto_${n}`))
        n += 1;
    return `MTProto_${n}`;
}
export function createDefaultMTProtoInstanceDraft(existingTags = []) {
    return {
...
export function createDefaultMTProtoCoreDraft() {
    return {
...
export function validateMTProtoInstanceDraft(draft, index, allTags, allPorts) {
    const issues = [];
...
export function validateMTProtoCoreDraft(draft) {
    const issues = [];
...
export function createMTProtoCoreConfigFromDraft(draft) {
...
export function generateMTProtoCoreConfigJsonFromDraft(draft, space = 2) {
...
export function createMTProtoInstanceConfigFromDraft(draft) {
